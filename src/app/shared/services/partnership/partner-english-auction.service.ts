...
// Abi
import { default as PartnerEnglishAuction } from 'src/app/shared/contracts/partners/PartnerEnglishAuction.json';

/**
 * Service to manage all functions related to the partner english auction smart contract
 */
@Injectable({
  providedIn: 'root'
})
export class PartnerEnglishAuctionService {
   ...
   */
  async putOnAuction(nftAddress: string, tokenId: number, amount: number, coin: string, auctionTime: number, bidPercentage: number, itemName: string, extraFeeTo: string, extraFeePercentage: number): Promise<any> {
    const transactionCoin = await this.tokenService.getCoinAddressByTicker(coin);
    const allowance = await this.tokenService.nftCheckAllowance(contractAddresses.partnerEnglishAuction, nftAddress);
    if (allowance) {
      ...
      try {
        ...
        const args = [nftAddress, tokenId, transactionCoin, auctionPrice, auctionTime, bidPercentage, extraFeeTo, extraFeePercentage];
        const tx = await this.connectionService.writeContract(contractAddresses.partnerEnglishAuction, PartnerEnglishAuction.abi, 'createNewNftAuction', args, customFee);
        dialog.close();
   ...
   * @param partnerEnglishAuction : contract of the partner english auction
  ...
  async getCustomAuctionFee(auctionTime: number): Promise<number> {
      ...
      const auctionData =  await this.connectionService.readContract(contractAddresses.partnerEnglishAuction, PartnerEnglishAuction.abi, 'mapAuctionTime', [auctionTime]);
      const price = auctionData.price;
      const fee = await this.connectionService.readContract(contractAddresses.partnerEnglishAuction, PartnerEnglishAuction.abi, 'convertBUSDToBNB', [price]);
      return fee;
    } else {
      return 0;
    }
  ...
  async addBid(nftAddress: string, tokenId: number, amount: number, coin: string, itemName: string): Promise<any> {
    ...
    const isApproved = await this.tokenService.tokenApprovement(contractAddresses.partnerEnglishAuction, userAddr, coin, bidAmount);
    if (isApproved) {
   ...
   * @param partnerEnglishAuction : the contract of partner english auction
  ...
  async makeRegularBid(nftAddress: string, tokenId: number, bidAmount: string, dialog: any): Promise<any> {
    try {
      const tx = await this.connectionService.writeContract(contractAddresses.partnerEnglishAuction, PartnerEnglishAuction.abi, 'makeBid', [nftAddress, tokenId, bidAmount]);
      dialog.close();
  ...
  async withdrawAuction(id: number, nftAddress: string): Promise<any> {
    ...
    try {
      const tx = await this.connectionService.writeContract(contractAddresses.partnerEnglishAuction, PartnerEnglishAuction.abi, 'settleAuction', [nftAddress, id]);
      dialog.close();
  ...
  async getAuctionData(id: number, nftAddress: string): Promise<any> {
    try {
      return await this.connectionService.readContract(contractAddresses.partnerEnglishAuction, PartnerEnglishAuction.abi, 'nftContractAuctions', [nftAddress, id]);
    } catch (error: any) {
      return '';
    }
  }
